...
impl Client {
    pub async fn connect<T: AsRef<str>>(addr: T, config: Option<ClientConfig>) -> Result<Self> {
        ...
        if !addr_str.starts_with("http://") && !addr_str.starts_with("https://") {
            return Err(Error::Protocol("URI must start with http:// or https://".to_string()));
        }

        let host = uri.host().ok_or_else(|| Error::Protocol("Missing host".to_string()))?;
        ...
        builder
            .initial_connection_window_size(config.max_message_size.try_into().map_err(|_| Error::Protocol("Size conversion failed".to_string()))?)
            .initial_window_size(config.max_message_size.try_into().map_err(|_| Error::Protocol("Size conversion failed".to_string()))?)
        ...
        let connection = Connection::new(transport)
            .with_max_frame_size(config.max_message_size);

        Ok(Self { connection, config })
    }

    pub async fn unary<T, U>(&mut self, method: &str, request: T, metadata: Option<Metadata>) 
        -> Result<Response<U>>
    where
        T: prost::Message,
        U: prost::Message + Default,
    {
        ...
        if let Some(response_data) = self.connection.receive().await? {
            ...
            let length = u32::from_be_bytes([
                ...
                response_data[3], response_data[4]
            ]) as usize;
